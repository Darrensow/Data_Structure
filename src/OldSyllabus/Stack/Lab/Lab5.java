...
//        }
        ...
        while(true){
            ...
            for(int i = currentPosition; i < n ; i++){                  //n = 4
...
//            System.out.println(str);
    ...
    public static String convertToInfix(String line){
        String [] operator = line.split(" "); //split the mathematical operation into array with ' '
        String str = "";
    ...
    public static String convertToPostfix(String infixExpression) {
        ...
        for (int i=0; i < temp.length; i++) {
            if (isInteger(temp[i])) {                       //if it is an integer
                str+=temp[i] + " ";
            } else if (temp[i].equals("(")) {               //if opening bracket
                num.push(temp[i]);
            } else if (temp[i].equals(")")) {               //if closing bracket
                something=num.pop();
                while (!something.equals("(")) {
                    str+=something + " ";
                    something=num.pop();
                }                                          //after while loop executed, the stack will be empty
            } else {
                if (!num.isEmpty()) {
                    if (num.peek().equals("("))
                        num.push(temp[i]);
                        ...
                        something=num.peek();
                        while (getPriority(temp[i]) <= getPriority(something)) { //when the operator at the top of the stack has higher priority
                            str+=num.pop() + " ";           //pops out the operator with higher priority
                            if (!num.isEmpty()) {           //stack not empty, may contain '('
                                something=num.peek();
                                if (something.equals("("))  //remains the '(' so that it can be closed afterwards at the second 'else-if' statement in the for loop
                                    break;
                            } else //stack is empty
                                break;
                        }
        ...
        while (!num.isEmpty()) {                            //pops out the remaining operator in the stack
            str+=num.pop() + " ";
        }
    ...
    public static int postfixEvaluation(String postfixExpression){
        ...
        for(int i = 0; i < temp.length; i++){
            if(isInteger(temp[i]))                          //push Integer into the stack
                num.push(Integer.parseInt(temp[i]));
            else                                            //pop two operands can operate on them with the operator(temp[i])
                num.push(getResult(num.pop(), num.pop(),temp[i]));
        }
        return num.pop();
    }
    //helper method to solve arithmetic operation
